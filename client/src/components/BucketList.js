...
const BucketList = () => {
  ...
  const completeBucketItem = (id) => {
    ...
    let updatedBucket = bucket.map((item) => {
      if (id === item.id) {
        ...
        item.eagerness = `complete ${item.eagerness}`;
        return updatedItem;
      }
      return item;
  ...
  const removeBucketItem = (id) => {
    ...
    setBucket((prev) => prev.filter((item) => id !== item.id));
  };
  ...
  return (
    <div>
      ...
      <Bucket bucket={bucket} completeBucketItem={completeBucketItem} removeBucketItem={removeBucketItem} editBucketItem={editBucketItem}></Bucket>
    </div>
  );
};
